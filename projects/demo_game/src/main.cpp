...
int main(int argc, char** argv) {
  ...
  rkg::log::init("rkg_demo_game", paths.root);
  ...
  if (!platform.init({1280, 720, "rkg_demo_game"})) {
...
#if RKG_ENABLE_IMGUI
  bool debug_ui_enabled = false;
  const bool debug_ui_requested =
      std::find(active_plugins.begin(), active_plugins.end(), "debug_ui_imgui") != active_plugins.end();
#endif
...
#if RKG_ENABLE_IMGUI
  if (debug_ui_enabled) {
    rkg::debug_ui::set_renderer_name(rkg::renderer_display_name(renderer_plugin));
  } else if (debug_ui_requested) {
    const auto title = std::string("rkg_demo_game (Debug UI unavailable for ") +
                       rkg::renderer_display_name(renderer_plugin) +
                       "; Vulkan-only)";
    if (auto* window = static_cast<SDL_Window*>(platform.native_window())) {
      SDL_SetWindowTitle(window, title.c_str());
    }
    rkg::log::warn(std::string("Debug UI unavailable for ") +
                   rkg::renderer_display_name(renderer_plugin) +
                   " (Vulkan-only).");
  }
#endif
  ...
  while (!platform.should_quit()) {
    ...
    float dt = platform.delta_seconds();
    if (fixed_dt > 0.0f) {
      dt = fixed_dt;
    }
    ...
    if (replay_active) {
      ...
      if (recorder.is_recording()) {
        recorder.record_frame(dt, input.states());
      }
    }
    ...
    bool reload_requested = raw_changed || action_state("Reload").pressed || auto_reload_requested;
#if RKG_ENABLE_IMGUI
    ...
    if (runtime.player != rkg::ecs::kInvalidEntity) {
      if (auto* transform = runtime.registry.get_transform(runtime.player)) {
        ...
        if (action_state("MoveForward").held) transform->position[1] += speed * dt;
        if (action_state("MoveBack").held) transform->position[1] -= speed * dt;
        if (action_state("MoveLeft").held) transform->position[0] -= speed * dt;
        if (action_state("MoveRight").held) transform->position[0] += speed * dt;
      }
    }
  }
